...
function remove-pkgs {
    $mcafeepkgs = @(
        ...
        foreach ($package in mcafeepkgs) {
            try {
                Write-Host "Attempting to uninstall package: $package"
                    $result = winget uninstall --id  $package -h
                    if ($result -match "Successfully uninstalled") {
                        Write-Host "Successfully uninstalled package: $package"
                    } else {
                        Write-Host "Unistalllation of package may not have succeeded" -ForegroundColor Yellow
                    }
            } catch {
                Write-Host "Failed to unistall $package: $_" -ForegroundColor Red
            }
        }
}
    ...
    Write-Host "===== Installing Software =====" -ForegroundColor DarkBlue
        ...
        foreach ($package in pkgs) {
            Write-Host "Installing package: $package"
                $result = winget install --id $package -h --accept-package-agreements --accept-source-agreements
        }
